...
ALLOWED_BASE_DIRS = [
    # reference folder (update if needed)
    Path(r"/home/antpc/Downloads/5000_RTH_Videos (Copy)").resolve(),
    # collected base
...
FRAMES_FPS = 12
FRAMES_MAX = 240          # cap extraction to prevent huge caches per clip
FRAMES_HEIGHT = 360       # moderate resolution (fast)
JPG_Q = 5                 # 2=high quality, 5=moderate, 10=low


app = FastAPI(title="ISL Annotation Tool (User-wise, Preview + Frames fallback)")
...
def ensure_frames(original: Path) -> Tuple[str, int]:
    """
    Extract low/medium resolution JPEG frames for viewing.
    Originals untouched. Cached in frames_cache/<key>/frame_000001.jpg...
    """
    ...
    try:
        # Decide how many frames to extract (cap)
        ...
        # Moderate viewing size + fps
        vf = f"fps={FRAMES_FPS},scale=-2:{FRAMES_HEIGHT}"

        # Important: output pattern
...
def user_word_detail(user_id: str, word: str):
    ...
    if not base:
        conn.close()
        raise HTTPException(status_code=404, detail=f"No data for user {user_id} and word {word}")
...
def media(request: Request, path: str = Query(...)):
    ...
    # For collected data: ALWAYS use preview for consistent playback
    if is_collected_path(p) or (codec is None) or (codec != "h264"):
        try:
            serve_path = ensure_h264_preview(p)
            if serve_path.stat().st_size == 0:
                raise RuntimeError("Preview is empty (0 bytes).")
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail=f"Preview generation failed for {p.name}. See ffmpeg_errors.log.\n{e}"
            )
    ...
    # No valid range -> send whole file (200), NEVER 416
    if parsed is None:
        if size == 0:
            raise HTTPException(status_code=500, detail="Serve file size is 0 bytes.")
        return StreamingResponse(
